...
func applyEtcdClusterDefaults(ec *EtcdCluster, log logr) {
	// When a TLS surface is present but no provider is named, the operator falls
	// back to the "auto" provider (matches the comment on TLSSurface.Provider).
	// Materializing it here means downstream code and validation see an explicit,
	// canonical value instead of "". Each surface (peer, client) is defaulted
	// independently because they carry their own provider.
	if ec.Spec.TLS != nil {
		defaultTLSSurfaceProvider(ec.Spec.TLS.Peer, "peer", log)
		defaultTLSSurfaceProvider(ec.Spec.TLS.Client, "client", log)
	}
}

// defaultTLSSurfaceProvider materializes the implicit "auto" provider on a single
// TLS surface, logging when it does so. A nil surface is left untouched.
func defaultTLSSurfaceProvider(s *TLSSurface, surface string, log logr) {
	if s == nil || strings.TrimSpace(s.Provider) != "" {
		return
	}
	s.Provider = tlsProviderAuto
	if log != nil {
		log.Info(fmt.Sprintf("defaulting spec.tls.%s.provider to %q (no provider specified)",
			surface, tlsProviderAuto))
	}
}
...
// and provider config, so the webhook validates each surface independently with the
// same per-surface coherence rules.
func validateTLS(tls *EtcdClusterTLS, path *field.Path) field.ErrorList {
	if tls == nil {
		return nil
	}
	var errs field.ErrorList
	errs = append(errs, validateTLSSurface(tls.Peer, path.Child("peer"))...)
	errs = append(errs, validateTLSSurface(tls.Client, path.Child("client"))...)
	return errs
}

// validateTLSSurface checks that a single TLS surface is internally coherent: the
// provider is one the operator understands, and the provider-specific config block
// required by that provider is present and complete.
func validateTLSSurface(tls *TLSSurface, path *field.Path) field.ErrorList {
	if tls == nil {
		return nil
	}
	var errs field.ErrorList
