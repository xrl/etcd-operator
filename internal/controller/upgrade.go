...
func desiredEtcdImage(ec *ecv1alpha1.EtcdCluster) string {
	return fmt.Sprintf("%s:%s", ec.Spec.ImageRegistry, etcdImageTag(ec.Spec.Version))
}
...
func (r *EtcdClusterReconciler) recoverDegradedUpgrade(ctx context.Context, s *reconcileState) (ctrl.Result, bool) {
	...
	sts, err := reconcileStatefulSet(ctx, logger, s.cluster, r.Client, *s.sts.Spec.Replicas, r.Scheme)
	if err != nil {
